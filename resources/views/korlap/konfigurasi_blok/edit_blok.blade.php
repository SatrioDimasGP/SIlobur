        ...
        <div class="card-body">
            <form action="{{ route('konfigurasi-blok.update', $blok->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" id="nama" value="{{ old('nama', $blok->nama) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <select name="lomba_id" id="lomba_id" class="form-select @error('lomba_id') is-invalid @enderror" required>
                        ...
                        @foreach ($lombas as $lomba)
                            <option value="{{ $lomba->id }}" {{ $lomba->id == $blok->lomba_id ? 'selected' : '' }}>{{ $lomba->nama }}</option>
                        @endforeach
                    </select>
