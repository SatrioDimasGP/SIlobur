...
<div class="container">
    ...
    @php
        ...
        $groupedBurung = $pemesanan->groupBy(function($item) {
            ...
            <ul>
                @foreach($groupedBurung as $jenisKelas => $items)
                    <li>
                        ...
                        No. Gantangan: {{ $items->pluck('gantangan.nomor')->filter()->map(fn($n) => $n)->implode(', ') }}
                    </li>
                @endforeach
            </ul>
            ...
            <div id="qr-section">
                @if ($allLunas && $transaksiDenganQr)
                    ...
                    @if ($transaksiDenganQr->qrcode->status_qr_id == 1)
                        <img src="{{ asset('storage/qr_code/' . $transaksiDenganQr->qrcode->file_qrcode) }}" alt="QR Code" width="200" id="qrcode-img">
                    @endif
