                        ...
                        @foreach($pemesanans as $createdAt => $group)
                            ...
                            <tr>
                                ...
                                <td>
                                    <ul class="mb-0 ps-3">
                                        @foreach($groupedBurung as $jenisKelas => $items)
                                            <li>{{ $items->pluck('gantangan.nomor')->filter()->map(fn($n) => 'No ' . $n)->implode(', ') }}</li>
                                        @endforeach
                                    </ul>
                                </td>
                                <td>
                                ...
                                <td>
                                    <a href="{{ route('pemesanans.show', ['created_at' => $createdAt]) }}" class="btn btn-sm btn-primary">Lihat Detail</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
